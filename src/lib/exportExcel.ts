...
export async function exportManagerWorkbook(from:string,to:string){
 const [tickets,comments,profiles,clients,inventory,movements,communications,history,audit,kpi]=await Promise.all([
 ...
 for(const q of [tickets,comments,profiles,clients,inventory,movements,communications,history,audit]) if(q.error) throw q.error
 ...
 add('Historique tickets',history.data||[])
 add('KPI Techniciens',((kpi.data as any)?.by_technician)||[])
