...
export async function exportManagerWorkbook(from:string,to:string){
 const [tickets,comments,profiles,clients,inventory,movements,audit,kpi]=await Promise.all([
 ...
 for(const q of [tickets,comments,profiles,clients,inventory,movements,audit]) if(q.error) throw q.error
