...
export function ActivityPage(){
 ...
 return <div className="page activity-page">
  ...
  <section className="grid two">
   <div className="card"><h3 className="section-title">Historique présence</h3><div className="activity-log-list">{presence.map((p:any)=><div className="activity-log-row" key={p.id}><div><b>{profiles.find(x=>x.id===p.technician_id)?.display_name||'Technicien'}</b><small>{new Date(p.started_at).toLocaleString('fr-FR')} → {p.ended_at?new Date(p.ended_at).toLocaleString('fr-FR'):'En cours'}</small></div><strong>{formatHours(hours(p.started_at,p.ended_at))}</strong></div>)}</div></div>
   <div className="card"><h3 className="section-title">Historique interventions</h3><div className="activity-log-list">{worklogs.map((w:any)=>{const t=tickets.find(x=>x.id===w.ticket_id);return <div className="activity-log-row" key={w.id}><div><b>{t?.ticket_number||'Ticket'} • {workLabels[w.work_type as WorkType]||w.work_type}</b><small>{new Date(w.started_at).toLocaleString('fr-FR')} → {w.ended_at?new Date(w.ended_at).toLocaleString('fr-FR'):'En cours'}</small></div><strong>{formatHours(hours(w.started_at,w.ended_at))}</strong></div>})}</div></div>
  </section>
 </div>
}
